...
class DataflowsConfigIsolationTests(unittest.TestCase):
    def setUp(self):
        ...
        # Témoins lus depuis la config, jamais codés en dur : ces tests portent
        # sur l'ISOLATION des dicts imbriqués, pas sur la chaîne de vendeurs.
        # V14 la fait évoluer (mt5,yfinance) et ces tests doivent y survivre.
        self.defauts = default_config.DEFAULT_CONFIG["data_vendors"]
    ...
    def test_get_config_returns_deep_copy(self):
        ...
        fresh = get_config()
        self.assertEqual(fresh["data_vendors"]["core_stock_apis"],
                         self.defauts["core_stock_apis"])
    ...
    def test_partial_nested_update_preserves_existing_defaults(self):
        ...
        fresh = get_config()
        self.assertEqual(fresh["data_vendors"]["core_stock_apis"], "alpha_vantage")
        for cle in ("technical_indicators", "fundamental_data", "news_data"):
            self.assertEqual(fresh["data_vendors"][cle], self.defauts[cle],
                             f"{cle} devait rester à son défaut après une MAJ partielle")
