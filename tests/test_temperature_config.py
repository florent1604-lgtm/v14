...
def _clear_temperature(monkeypatch):
    """Vide la temperature sur TOUS les prefixes reconnus par V14.

    Le .env du projet definit TITANIUM_TEMPERATURE : ne retirer que la
    variante TRADINGAGENTS_ laisserait la config du projet fuiter ici.
    """
    for var in ("TITANIUM_TEMPERATURE", "TRADINGAGENTS_TEMPERATURE"):
...
class TestTemperatureEnvOverlay:
    def test_env_sets_temperature(self, monkeypatch):
        ...
        _clear_temperature(monkeypatch)
        monkeypatch.setenv("TITANIUM_TEMPERATURE", "0.2")
        importlib.reload(dc)
