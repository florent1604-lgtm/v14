...
class TradingAgentsGraph:
    ...
    def propagate(self, company_name, trade_date, asset_type: str = "stock",
        ...
        ``extra_context`` is appended to the instrument context seen by every
        agent. Titanium V14 uses it to hand the analysts its deterministic
        reading of the setup — pillars, zones, indicators — so they judge that
        reading against their own research instead of working blind. It is
        appended rather than substituted: the resolved ticker identity stays
        authoritative, and a malformed brief can never erase it.
    ...
    def _run_graph(self, company_name, trade_date, asset_type: str = "stock",
        ...
        if extra_context:
            # Appended, never substituted: the resolved ticker identity must
            # stay authoritative even if the caller's brief is malformed.
            instrument_context = (
                f"{instrument_context}\n\n"
                "--- Lecture déterministe fournie par Titanium V14 ---\n"
                f"{extra_context}")
